...
\begin{itemize}
    ...
    \end{enumerate}
    \item Let $V,W$ be finite-dimensional vector spaces over $\R$. We let $\lin{V}{W}$ be the vector space of all linear transformations $\phi:V\to W$.
    \item If we pick bases $N_1,\dots,N_n$ of $V$ and $w_1,\dots,w_m$ of $W$, then $V\cong\R^n$ and $W\cong\R^m$. It follows that $\lin{V}{W}\cong\R^{mn}$.
    \item $\lin{V}{W}\times\lin{W}{U}\xrightarrow{\text{compose}}\lin{V}{U}$, i.e., $\R^{mn}\times\R^{nl}\xrightarrow[\text{mult.}]{\text{matrix}}\R^{ml}$.
    ...
    \begin{itemize}
        ...
        \begin{enumerate}
            ...
            \item $\norm{AB}\leq\norm{A}\norm{B}$.
        \end{enumerate}
        \item Note that we get a metric space structure on $\lin{V}{W}$ by defining $d(A,B)=\norm{A-B}$.
    \end{itemize}
