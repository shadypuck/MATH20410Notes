...
\begin{itemize}
    ...
    \end{itemize}
    \item \marginnote{3/11:}Analysis I:
    \begin{itemize}
        \item $\R$.
        \item Metric spaces.
    ...
    \begin{itemize}
        ...
        \item $\mu=0$.
        \begin{itemize}
            \item Suppose (contradiction): $p(z_0)\neq 0$.
            \item Trick: Define
            \begin{equation*}
                Q(z) = \frac{p(z+z_0)}{p(z_0)}
            \end{equation*}
            \item $Q(0)=1$ and $Q(z)\geq 1$ (this second statement is what we're gonna contradict).
            \item Write
            \begin{equation*}
                Q(z) = 1+b_kz^k+\cdots+b_nz^n
            \end{equation*}
            \item Goal: For $r$ sufficiently small ($r>0$), the right term below is less than $|b_k|$, so it is positive, meaning that $Q$ is less than or equal to 1 minus a positive number, i.e., $Q<1$, a contradiction.
            \begin{align*}
                |Q(r\e[i\theta])| &\leq 1-r^k[|b_k|-r|b_{k+1}|-\cdots-r^{n-k}|b_n|]
            \end{align*}
        \end{itemize}
        \item Proof of goal: There exists $\theta\in[0,2\pi)$ such that $b_k\e[ik\theta]=-|b_k|$. Take $r$ small enough so $r^k|b_k|<1$, so by the triangle inequality together with the fact that $|1+b_kr^k\e[ik\theta]|=1-r^k|b_k|$.
    \end{itemize}
    \item Corollary: Polynomials over $\C$ factor completely.
\end{itemize}
